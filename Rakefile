...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/ghiculescu/text_splitters/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("text_splitters.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in text_splitters.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "text_splitters.gemspec", /ruby_version = .*">= (.*)"/ => lowest_minor
